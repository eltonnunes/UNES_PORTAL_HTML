...
O component `mediaGrid` utiliza o seletor `<mediaGrid> </mediaGrid>` que possui valores default, dados, paginação e filtros.
...
- `rows`: O atributo `rows` é do tipo `Array[Object]` e deve conter as linhas a serem exibidas, os keys do header devem ser utilizados como key dos objects contidos no array `mediaGrid`.
...
- `pagination`: O atributo `pagination` é do tipo `Object` e deve conter os keys `[current, totalRecords, recordPerPage, previous, next]` com seus respectivos values.
  ...
  * `recordPerPage` é do tipo `number` e deve conter o total de registros a ser exibido no `mediaGrid`.
  * `previous` é do tipo `boolean` e deve conter `true / false`, possibilita a exibição ou não do botão voltar no `mediaGrid`.
  * `next` é do tipo `boolean` e deve conter `true / false`, possibilita a exibição ou não do botão avançar no `mediaGrid`.
###### Exemplo de utilização
```javascript
          pagination = {
                          current       : 4,
